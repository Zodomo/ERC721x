...
contract ERC721xEnumUpgradeable is ERC721EnumerableUpgradeable, LockRegistry {
	...
	function transferFrom(address _from, address _to, uint256 _tokenId) public override(ERC721EnumerableUpgradeable) virtual {
		require(isUnlocked(_tokenId), "Token is locked");
		ERC721EnumerableUpgradeable.transferFrom(_from, _to, _tokenId);
	}

	function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes memory _data) public override(ERC721EnumerableUpgradeable) virtual {
		require(isUnlocked(_tokenId), "Token is locked");
		ERC721EnumerableUpgradeable.safeTransferFrom(_from, _to, _tokenId, _data);
	}
