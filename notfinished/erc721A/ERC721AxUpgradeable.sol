...
contract ERC721AxUpgradeable is ERC721AUpgradeable, LockRegistry {
    ...
    function __ERC721Ax_init(string memory name_, string memory symbol_) internal onlyInitializingERC721A {
	...
	function transferFrom(address _from, address _to, uint256 _tokenId) public payable override(ERC721AUpgradeable) virtual {
		require(isUnlocked(_tokenId), "Token is locked");
		ERC721AUpgradeable.transferFrom(_from, _to, _tokenId);
	}

	function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes memory _data) public payable override(ERC721AUpgradeable) virtual {
		require(isUnlocked(_tokenId), "Token is locked");
